...
| 文件                                                            | 职责                                                                                                                                                                        |
| --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/game/types.ts`、`content.ts`                               | 状态契约、30 关与 12 种升级配置                                                                                                                                             |
| `src/game/roguelike.ts`                                         | Buff 等级、确定性三选一、全局进度与收益文案                                                                                                                                 |
| `src/game/difficulty.ts`                                        | 四档难度、生命成长、攻击压力与恢复规则                                                                                                                                      |
| `src/i18n/`                                                     | 中英目录、纯翻译函数、语言偏好与文档元数据；不接入模拟                                                                                                                      |
...
| `src/game/cities.ts`                                            | 五地三十个街段的程序场景与城市细节                                                                                                                                          |
| `src/game/input.ts`、`audio.ts`                                 | 键盘/触控输入、合成音频                                                                                                                                                     |
| `src/game/storage.ts`                                           | v1 兼容读取、v2 小关/选卡及难度存档校验；按难度记录最高分                                                                                                                   |
| `src/game/offline.ts`                                           | Service Worker 注册与离线缓存状态                                                                                                                                           |
| `src/components/game/`                                          | 菜单、HUD、弹层、触控；StreetPrompt 提示前进与追击；GameShell 协调生命周期；BuffDraft 三选一；RunBuild 展示叠加构筑；DifficultyPicker 选择新局难度；LanguageSelect 切换语言 |
